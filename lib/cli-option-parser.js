...
/**
 * Parse CLI arguments to options that Marksnap can understand.
 */
...
CLIOptionParser.getDest = function (cwd, argv, toType) {
  ...
  if (argv._[1]) {
    ...
    dest = path.isAbsolute(paramDist) ?
           paramDist :
           path.resolve(cwd, paramDist);
  ...
  if (toType == 'pdf' && path.extname(dest).toLowerCase() !== '.pdf') {
    dest += '.pdf';
  }
